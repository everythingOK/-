...
ApiRobot.prototype.send = function (erpMsgUrl, msg, callback) {
    logger.enter();
    logger.trace('SCC will Send msg :' + JSON.stringify(msg));
    superAgent
        .post(erpMsgUrl)
...
ApiRobot.prototype.sendMsg = function () {
    ...
    dbService.retrieveUserErpInfo(cloudDbName, enterpriseId, function (error, result) {
        ...
        if (result && result[0]) {
            ...
            erpAppCodeUrl = result[0].erpAppCodeUrl || undefined;
            erpMsgUrl = result[0].erpMsgUrl || undefined;
            appKey = result[0].appKey || undefined;
            customerDbName = __customerDBPrefix + "_" + customerDBSuffix;
        } else {
            return callback(new Error("该客户（商户）没有配置appkey"));
        }
